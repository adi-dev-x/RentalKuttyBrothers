...
func AdminAuthentication(tokenString, jwtKey string) (model.UserClaims, error) {
...
func (s Adminjwt) AdminAuthMiddleware() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			claims, err := AdminAuthentication(tokenString, s.Config.AdJWTKey)
			if err != nil {
				return c.JSON(http.StatusUnauthorized, map[string]string{"error": "invalid token"})
			}
			if claims.Role == constants.IRRL_ROLE || claims.Role == constants.ADMIN_ROLE {

				c.Set("username", claims.Username)
				return next(c)
			} else {
				return c.JSON(http.StatusUnauthorized, map[string]string{"error": "invalid user access"})
			}
